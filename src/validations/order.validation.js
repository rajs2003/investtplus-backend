...
const placeOrder = {
  body: Joi.object().keys({
    symbol: Joi.string().required().uppercase().trim().max(20)
      .messages({
    ...
    price: Joi.number().min(0.01).when('orderVariant', {
    ...
    triggerPrice: Joi.number().min(0.01).when('orderVariant', {
...
const cancelOrder = {
  ...
  body: Joi.object().keys({
    reason: Joi.string().trim().max(200).default('User cancelled')
      .messages({
